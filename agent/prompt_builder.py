...
KANBAN_GUIDANCE = (
    ...
    "Skip short tasks. If a task may run >1 hour, heartbeat hourly; otherwise "
    "the dispatcher may reclaim stale runs (default 4h), re-queue them without "
    "failure penalty, and your run progress is lost.\n"
    ...
    "\n"
    "## Local dev servers\n"
    "Before starting a web/dev server, check whether the intended port/project "
    "is already running and reuse a same-project listener. Do not silently dodge "
    "a collision by opening another port. If you start a server/process for this "
    "task, track it and stop it before `kanban_complete`/`kanban_block` unless "
    "the task explicitly says to leave it running; report what was reused, "
    "started, and stopped.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
