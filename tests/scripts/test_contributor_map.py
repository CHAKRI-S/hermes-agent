...
def test_add_contributor_refuses_a_case_collision(tmp_path, monkeypatch):
    ...
    # The refusal is host-independent (mapping identity is case-insensitive by
    # policy), so NO new file was created — only the original mapping remains.
    # (An `exists()` probe of the lowercase name would false-fail on
    # case-insensitive filesystems, where the original casts that shadow.)
