...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    # macOS may clear setgid when the parent group is outside the test user's
    # groups, but retains it when the temp directory inherits the user's group.
    ...
    assert stat.S_IMODE(event.stat().st_mode) in expected_event_modes, (
        ...
        f"{sorted(map(oct, expected_event_modes))}"
    )
...
def test_seed_supervise_skeleton_handles_log_subservice(tmp_path) -> None:
    ...
    assert stat.S_IMODE(log_supervise_event.stat().st_mode) in expected_event_modes
