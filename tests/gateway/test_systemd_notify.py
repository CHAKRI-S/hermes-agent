...
def test_notify_sends_real_unix_datagram(monkeypatch):
    ...
    # default nested temp path can exceed that limit, so keep the socket in a
    # dedicated short-lived directory under the canonical temp root.
...
@pytest.mark.skipif(
    ...
    reason="Linux abstract namespace sockets are unavailable",
)
